...
errors_all_fold <- loo_fold %>% arrange(upr) %>%
  ...
  geom_point(aes(color = test), size = 0.8) +
  ...
  annotate("text", x = 100, y = 12.5, label = "Fold data", size = 5) +
  theme(legend.position = c(0.63,0.8), legend.title = element_blank())
errors_all_bind <- loo_bind %>% arrange(upr) %>%
  ...
  scale_x_continuous(NULL, breaks = NULL) +
  labs(y = "Error (kcal/mol)") +
  annotate("text", x = 78, y = 15.7, label = "Bind data", size = 5) +
  theme(legend.position = c(0.63,0.8), legend.title = element_blank())
```
...
errors_sys_fold <- loo_fold %>% arrange(upr) %>%
  ...
  geom_point(aes(color=test), size = .5, show.legend = FALSE) +
...
ggsave("../figures/errors_sorted.pdf", width = 170, height = 140, units = "mm")
```
...
ggsave("../figures/classifications_separated.pdf", width = 170, height = 208, units = "mm", dpi = 300)
#ggsave("../figures/classifications_separated.pdf", width = 9, height = 11, dpi = 300)
```
...
ggsave("../figures/classifications_resampled.pdf", width = 170, height = 141.5, units = "mm", dpi = 300)
```
